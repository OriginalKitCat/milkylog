		...
		if (selectedProjects && selectedProjects.trim()) {
			try {
				...
				try {
					...
					return json(
						{ error: `Failed to fetch today Hackatime data for devlog: ${hackatimeError.message || devlogError}` },
						{ status: 400 }
					);
				}
				...
				try {
					...
					return json(
						{ error: `Failed to fetch today Hackatime data for devlog: ${hackatimeError.message || hackatimeError}` },
						{ status: 400 }
					);
				}
				...
				if (allProjects.length > 0) {
					try {
						...
						return json(
						{ error: `Failed to fetch today Hackatime data for devlog: ${hackatimeError.message || artlogError}` },
						{ status: 400 }
					);
					}
				}
				...
				return json(
					{ error: `Failed to fetch projects for devlog hours calculation: ${hackatimeError.message || projectError}` },
					{ status: 400 }
				);
